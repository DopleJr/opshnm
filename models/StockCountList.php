 ...
 * Page class
 */
class StockCountList extends StockCount
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'stock_count';

    // Page object name
    public $PageObjName = "StockCountList";
    ...
    public $FormName = "fstock_countlist";
    ...
    public function __construct()
    {
        ...
        // Table object (stock_count)
        if (!isset($GLOBALS["stock_count"]) || get_class($GLOBALS["stock_count"]) == PROJECT_NAMESPACE . "stock_count") {
            $GLOBALS["stock_count"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "stockcountadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "stockcountdelete";
        $this->MultiUpdateUrl = "stockcountupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'stock_count');
        }
    ...
    protected function hideFieldsForAddEdit()
    {
        ...
        if ($this->isAddOrEdit()) {
            $this->time_created->Visible = false;
        }
    }
    ...
    public $PageSizes = "10,20,50"; // Page sizes (comma separated)
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->get_total->AdvancedSearch->toJson(), ","); // Field get_total
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fstock_countsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field get_total
        $this->get_total->AdvancedSearch->SearchValue = @$filter["x_get_total"];
        $this->get_total->AdvancedSearch->SearchOperator = @$filter["z_get_total"];
        $this->get_total->AdvancedSearch->SearchCondition = @$filter["v_get_total"];
        $this->get_total->AdvancedSearch->SearchValue2 = @$filter["y_get_total"];
        $this->get_total->AdvancedSearch->SearchOperator2 = @$filter["w_get_total"];
        $this->get_total->AdvancedSearch->save();
    ...
    protected function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->time_created, $default, true); // time_created
        $this->buildSearchSql($where, $this->get_total, $default, false); // get_total
        ...
        if (!$default && $this->Command == "search") {
            ...
            $this->time_created->AdvancedSearch->save(); // time_created
            $this->get_total->AdvancedSearch->save(); // get_total
        }
        return $where;
    }
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->time_created->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->get_total->AdvancedSearch->issetSession()) {
            return true;
        }
        return false;
    }
    ...
    protected function resetAdvancedSearchParms()
    {
        ...
        $this->time_created->AdvancedSearch->unsetSession();
        $this->get_total->AdvancedSearch->unsetSession();
    }
    ...
    protected function restoreSearchParms()
    {
        ...
        $this->time_created->AdvancedSearch->load();
        $this->get_total->AdvancedSearch->load();
    }
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fstock_countlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fstock_countlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<button type=\"button\" class=\"ew-action ew-multi-delete\" title=\"" . HtmlTitle($Language->phrase("DeleteSelectedLink")) . "\" data-caption=\"" . HtmlTitle($Language->phrase("DeleteSelectedLink")) . "\" form=\"fstock_countlist\" data-ew-action=\"submit\" data-url=\"" . GetUrl($this->MultiDeleteUrl) . "\"data-data='{\"action\":\"show\"}'>" . $Language->phrase("DeleteSelectedLink") . "</button>";
        $item->Visible = $Security->canDelete();
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fstock_countsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fstock_countsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fstock_countlist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    protected function loadSearchValues()
    {
        ...
        // time_created
        if ($this->time_created->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->time_created->AdvancedSearch->SearchValue != "" || $this->time_created->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // get_total
        if ($this->get_total->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->get_total->AdvancedSearch->SearchValue != "" || $this->get_total->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        return $hasValue;
    ...
    public function renderRow()
    {
        ...
        // time_created
        $this->time_created->CellCssStyle = "white-space: nowrap;";

        // get_total
        $this->get_total->CellCssStyle = "white-space: nowrap;";
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // time_created
            if ($this->time_created->UseFilter && !EmptyValue($this->time_created->AdvancedSearch->SearchValue)) {
                if (is_array($this->time_created->AdvancedSearch->SearchValue)) {
                    $this->time_created->AdvancedSearch->SearchValue = implode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->time_created->AdvancedSearch->SearchValue);
                }
                $this->time_created->EditValue = explode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->time_created->AdvancedSearch->SearchValue);
            }
        }
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="fstock_countlist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fstock_countsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
        ...
        if (IsMobile()) {
            $item->Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" . $Language->phrase("AdvancedSearch") . "\" data-caption=\"" . $Language->phrase("AdvancedSearch") . "\" href=\"stockcountsearch\">" . $Language->phrase("AdvancedSearchBtn") . "</a>";
        } else {
            $item->Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" . $Language->phrase("AdvancedSearch") . "\" data-table=\"stock_count\" data-caption=\"" . $Language->phrase("AdvancedSearch") . "\" data-ew-action=\"modal\" data-url=\"stockcountsearch\" data-btn=\"SearchBtn\">" . $Language->phrase("AdvancedSearchBtn") . "</a>";
        }
        $item->Visible = true;
