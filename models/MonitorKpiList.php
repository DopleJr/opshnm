 ...
 * Page class
 */
class MonitorKpiList extends MonitorKpi
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'monitor_kpi';

    // Page object name
    public $PageObjName = "MonitorKpiList";
    ...
    public $FormName = "fmonitor_kpilist";
    ...
    public function __construct()
    {
        ...
        // Table object (monitor_kpi)
        if (!isset($GLOBALS["monitor_kpi"]) || get_class($GLOBALS["monitor_kpi"]) == PROJECT_NAMESPACE . "monitor_kpi") {
            $GLOBALS["monitor_kpi"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "monitorkpiadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "monitorkpidelete";
        $this->MultiUpdateUrl = "monitorkpiupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'monitor_kpi');
        }
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->Done->AdvancedSearch->toJson(), ","); // Field Done
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fmonitor_kpisrch", $filters);
    ...
    protected function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->Subject, $default, true); // Subject
        $this->buildSearchSql($where, $this->Divisi, $default, true); // Divisi
        $this->buildSearchSql($where, $this->Pending, $default, true); // Pending
        $this->buildSearchSql($where, $this->Done, $default, true); // Done
        ...
        if (!$default && $this->Command == "search") {
            ...
            $this->Done->AdvancedSearch->save(); // Done
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->Done->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    protected function resetAdvancedSearchParms()
    {
        ...
        $this->Done->AdvancedSearch->unsetSession();
    ...
    protected function setupSortOrder()
    {
        ...
        if ($this->Command != "json") {
            $defaultSort = $this->Date->Expression . " DESC"; // Set up default sort
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fmonitor_kpilist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fmonitor_kpilist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("Pending", $this->createColumnOption("Pending"));
            $option->add("Done", $this->createColumnOption("Done"));
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fmonitor_kpisrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fmonitor_kpisrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fmonitor_kpilist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    protected function loadSearchValues()
    {
        ...
        // Done
        if ($this->Done->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->Done->AdvancedSearch->SearchValue != "" || $this->Done->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    protected function newRow()
    {
        ...
        $row['Done'] = $this->Done->DefaultValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // Subject
            if ($this->Subject->UseFilter && !EmptyValue($this->Subject->AdvancedSearch->SearchValue)) {
                if (is_array($this->Subject->AdvancedSearch->SearchValue)) {
                    $this->Subject->AdvancedSearch->SearchValue = implode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->Subject->AdvancedSearch->SearchValue);
                }
                $this->Subject->EditValue = explode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->Subject->AdvancedSearch->SearchValue);
            }
            ...
            // Pending
            if ($this->Pending->UseFilter && !EmptyValue($this->Pending->AdvancedSearch->SearchValue)) {
                if (is_array($this->Pending->AdvancedSearch->SearchValue)) {
                    $this->Pending->AdvancedSearch->SearchValue = implode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->Pending->AdvancedSearch->SearchValue);
                }
                $this->Pending->EditValue = explode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->Pending->AdvancedSearch->SearchValue);
            }

            // Done
            if ($this->Done->UseFilter && !EmptyValue($this->Done->AdvancedSearch->SearchValue)) {
                if (is_array($this->Done->AdvancedSearch->SearchValue)) {
                    $this->Done->AdvancedSearch->SearchValue = implode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->Done->AdvancedSearch->SearchValue);
                }
                $this->Done->EditValue = explode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->Done->AdvancedSearch->SearchValue);
            }
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="fmonitor_kpilist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fmonitor_kpisrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
