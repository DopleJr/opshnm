 ...
 * Page class
 */
class EmptyBoxList extends EmptyBox
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'empty_box';

    // Page object name
    public $PageObjName = "EmptyBoxList";
    ...
    public $FormName = "fempty_boxlist";
    ...
    public function __construct()
    {
        ...
        // Table object (empty_box)
        if (!isset($GLOBALS["empty_box"]) || get_class($GLOBALS["empty_box"]) == PROJECT_NAMESPACE . "empty_box") {
            $GLOBALS["empty_box"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "emptyboxadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "emptyboxdelete";
        $this->MultiUpdateUrl = "emptyboxupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'empty_box');
        }
    ...
    public function run()
    {
        ...
        $this->id->Visible = false;
        $this->photo_before->setVisibility();
        $this->photo_after->setVisibility();
        ...
        // Set up lookup cache
        $this->setupLookupOptions($this->divisi);
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->photo_before->AdvancedSearch->toJson(), ","); // Field photo_before
        $filterList = Concat($filterList, $this->photo_after->AdvancedSearch->toJson(), ","); // Field photo_after
        $filterList = Concat($filterList, $this->divisi->AdvancedSearch->toJson(), ","); // Field divisi
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fempty_boxsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field photo_before
        $this->photo_before->AdvancedSearch->SearchValue = @$filter["x_photo_before"];
        $this->photo_before->AdvancedSearch->SearchOperator = @$filter["z_photo_before"];
        $this->photo_before->AdvancedSearch->SearchCondition = @$filter["v_photo_before"];
        $this->photo_before->AdvancedSearch->SearchValue2 = @$filter["y_photo_before"];
        $this->photo_before->AdvancedSearch->SearchOperator2 = @$filter["w_photo_before"];
        $this->photo_before->AdvancedSearch->save();

        // Field photo_after
        $this->photo_after->AdvancedSearch->SearchValue = @$filter["x_photo_after"];
        $this->photo_after->AdvancedSearch->SearchOperator = @$filter["z_photo_after"];
        $this->photo_after->AdvancedSearch->SearchCondition = @$filter["v_photo_after"];
        $this->photo_after->AdvancedSearch->SearchValue2 = @$filter["y_photo_after"];
        $this->photo_after->AdvancedSearch->SearchOperator2 = @$filter["w_photo_after"];
        $this->photo_after->AdvancedSearch->save();

        // Field divisi
        $this->divisi->AdvancedSearch->SearchValue = @$filter["x_divisi"];
        $this->divisi->AdvancedSearch->SearchOperator = @$filter["z_divisi"];
        $this->divisi->AdvancedSearch->SearchCondition = @$filter["v_divisi"];
        $this->divisi->AdvancedSearch->SearchValue2 = @$filter["y_divisi"];
        $this->divisi->AdvancedSearch->SearchOperator2 = @$filter["w_divisi"];
        $this->divisi->AdvancedSearch->save();
    ...
    protected function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->photo_before, $default, true); // photo_before
        $this->buildSearchSql($where, $this->photo_after, $default, true); // photo_after
        $this->buildSearchSql($where, $this->divisi, $default, true); // divisi
        ...
        if (!$default && $this->Command == "search") {
            ...
            $this->photo_before->AdvancedSearch->save(); // photo_before
            $this->photo_after->AdvancedSearch->save(); // photo_after
            $this->divisi->AdvancedSearch->save(); // divisi
    ...
    protected function basicSearchWhere($default = false)
    {
        ...
        // Fields to search
        $searchFlds = [];
        $searchFlds[] = &$this->photo_before;
        $searchFlds[] = &$this->photo_after;
        $searchFlds[] = &$this->divisi;
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->photo_before->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->photo_after->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->divisi->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    protected function resetAdvancedSearchParms()
    {
        ...
        $this->photo_before->AdvancedSearch->unsetSession();
        $this->photo_after->AdvancedSearch->unsetSession();
        $this->divisi->AdvancedSearch->unsetSession();
    ...
    protected function restoreSearchParms()
    {
        ...
        $this->photo_before->AdvancedSearch->load();
        $this->photo_after->AdvancedSearch->load();
        $this->divisi->AdvancedSearch->load();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->photo_before); // photo_before
            $this->updateSort($this->photo_after); // photo_after
            $this->updateSort($this->divisi); // divisi
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->id->setSort("");
                $this->photo_before->setSort("");
                $this->photo_after->setSort("");
                $this->divisi->setSort("");
                $this->status->setSort("");
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fempty_boxlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fempty_boxlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<button type=\"button\" class=\"ew-action ew-multi-delete\" title=\"" . HtmlTitle($Language->phrase("DeleteSelectedLink")) . "\" data-caption=\"" . HtmlTitle($Language->phrase("DeleteSelectedLink")) . "\" form=\"fempty_boxlist\" data-ew-action=\"submit\" data-url=\"" . GetUrl($this->MultiDeleteUrl) . "\"data-data='{\"action\":\"show\"}'>" . $Language->phrase("DeleteSelectedLink") . "</button>";
        $item->Visible = $Security->canDelete();
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("photo_before", $this->createColumnOption("photo_before"));
            $option->add("photo_after", $this->createColumnOption("photo_after"));
            $option->add("divisi", $this->createColumnOption("divisi"));
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fempty_boxsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fempty_boxsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fempty_boxlist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    protected function loadSearchValues()
    {
        ...
        // photo_before
        if ($this->photo_before->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->photo_before->AdvancedSearch->SearchValue != "" || $this->photo_before->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // photo_after
        if ($this->photo_after->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->photo_after->AdvancedSearch->SearchValue != "" || $this->photo_after->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // divisi
        if ($this->divisi->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->divisi->AdvancedSearch->SearchValue != "" || $this->divisi->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    public function loadRowValues($rs = null)
    {
        ...
        $this->photo_before->setDbValue($this->photo_before->Upload->DbValue);
        $this->photo_after->Upload->DbValue = $row['photo_after'];
        $this->photo_after->setDbValue($this->photo_after->Upload->DbValue);
    ...
    protected function newRow()
    {
        ...
        $row['photo_before'] = $this->photo_before->DefaultValue;
        $row['photo_after'] = $this->photo_after->DefaultValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($this->photo_before->UseColorbox) {
                ...
                $this->photo_before->LinkAttrs["data-rel"] = "empty_box_x" . $this->RowCount . "_photo_before";
            ...
            if ($this->photo_after->UseColorbox) {
                ...
                $this->photo_after->LinkAttrs["data-rel"] = "empty_box_x" . $this->RowCount . "_photo_after";
            ...
            // photo_before
            if ($this->photo_before->UseFilter && !EmptyValue($this->photo_before->AdvancedSearch->SearchValue)) {
                if (is_array($this->photo_before->AdvancedSearch->SearchValue)) {
                    $this->photo_before->AdvancedSearch->SearchValue = implode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->photo_before->AdvancedSearch->SearchValue);
                }
                $this->photo_before->EditValue = explode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->photo_before->AdvancedSearch->SearchValue);
            }

            // photo_after
            if ($this->photo_after->UseFilter && !EmptyValue($this->photo_after->AdvancedSearch->SearchValue)) {
                if (is_array($this->photo_after->AdvancedSearch->SearchValue)) {
                    $this->photo_after->AdvancedSearch->SearchValue = implode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->photo_after->AdvancedSearch->SearchValue);
                }
                $this->photo_after->EditValue = explode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->photo_after->AdvancedSearch->SearchValue);
            }

            // divisi
            if ($this->divisi->UseFilter && !EmptyValue($this->divisi->AdvancedSearch->SearchValue)) {
                if (is_array($this->divisi->AdvancedSearch->SearchValue)) {
                    $this->divisi->AdvancedSearch->SearchValue = implode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->divisi->AdvancedSearch->SearchValue);
                }
                $this->divisi->EditValue = explode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->divisi->AdvancedSearch->SearchValue);
            }
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="fempty_boxlist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fempty_boxsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
