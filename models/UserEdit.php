...
class UserEdit extends User
{
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'ip_loggedin' first before field var 'x_ip_loggedin'
        $val = $CurrentForm->hasValue("ip_loggedin") ? $CurrentForm->getValue("ip_loggedin") : $CurrentForm->getValue("x_ip_loggedin");
        if (!$this->ip_loggedin->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->ip_loggedin->Visible = false; // Disable update for API request
            } else {
                $this->ip_loggedin->setFormValue($val);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->ip_loggedin->CurrentValue = $this->ip_loggedin->FormValue;
    ...
    protected function validateForm()
    {
        ...
        if ($this->ip_loggedin->Required) {
            if (!$this->ip_loggedin->IsDetailKey && EmptyValue($this->ip_loggedin->FormValue)) {
                $this->ip_loggedin->addErrorMessage(str_replace("%s", $this->ip_loggedin->caption(), $this->ip_loggedin->RequiredErrorMessage));
            }
        }
    ...
    protected function editRow()
    {
        ...
        // ip_loggedin
        $this->ip_loggedin->CurrentValue = CurrentUserIP();
        $this->ip_loggedin->setDbValueDef($rsnew, $this->ip_loggedin->CurrentValue, null);
