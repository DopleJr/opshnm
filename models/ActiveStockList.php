 ...
 * Page class
 */
class ActiveStockList extends ActiveStock
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'active_stock';

    // Page object name
    public $PageObjName = "ActiveStockList";
    ...
    public $FormName = "factive_stocklist";
    ...
    public function __construct()
    {
        ...
        // Table object (active_stock)
        if (!isset($GLOBALS["active_stock"]) || get_class($GLOBALS["active_stock"]) == PROJECT_NAMESPACE . "active_stock") {
            $GLOBALS["active_stock"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "activestockadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "activestockdelete";
        $this->MultiUpdateUrl = "activestockupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'active_stock');
        }
    ...
    public function run()
    {
        ...
        $this->active_bin->setVisibility();
        $this->total_stock->setVisibility();
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->active_bin->AdvancedSearch->toJson(), ","); // Field active_bin
        $filterList = Concat($filterList, $this->total_stock->AdvancedSearch->toJson(), ","); // Field total_stock
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "factive_stocksrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field active_bin
        $this->active_bin->AdvancedSearch->SearchValue = @$filter["x_active_bin"];
        $this->active_bin->AdvancedSearch->SearchOperator = @$filter["z_active_bin"];
        $this->active_bin->AdvancedSearch->SearchCondition = @$filter["v_active_bin"];
        $this->active_bin->AdvancedSearch->SearchValue2 = @$filter["y_active_bin"];
        $this->active_bin->AdvancedSearch->SearchOperator2 = @$filter["w_active_bin"];
        $this->active_bin->AdvancedSearch->save();

        // Field total_stock
        $this->total_stock->AdvancedSearch->SearchValue = @$filter["x_total_stock"];
        $this->total_stock->AdvancedSearch->SearchOperator = @$filter["z_total_stock"];
        $this->total_stock->AdvancedSearch->SearchCondition = @$filter["v_total_stock"];
        $this->total_stock->AdvancedSearch->SearchValue2 = @$filter["y_total_stock"];
        $this->total_stock->AdvancedSearch->SearchOperator2 = @$filter["w_total_stock"];
        $this->total_stock->AdvancedSearch->save();
    ...
    protected function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->active_bin, $default, true); // active_bin
        $this->buildSearchSql($where, $this->total_stock, $default, true); // total_stock
        ...
        if (!$default && $this->Command == "search") {
            ...
            $this->active_bin->AdvancedSearch->save(); // active_bin
            $this->total_stock->AdvancedSearch->save(); // total_stock
        }
        return $where;
    }
    ...
    protected function basicSearchWhere($default = false)
    {
        ...
        $searchFlds[] = &$this->active_bin;
        $searchFlds[] = &$this->total_stock;
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->active_bin->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->total_stock->AdvancedSearch->issetSession()) {
            return true;
        }
        return false;
    }
    ...
    protected function resetAdvancedSearchParms()
    {
        ...
        $this->active_bin->AdvancedSearch->unsetSession();
        $this->total_stock->AdvancedSearch->unsetSession();
    }
    ...
    protected function restoreSearchParms()
    {
        ...
        $this->active_bin->AdvancedSearch->load();
        $this->total_stock->AdvancedSearch->load();
    }
    ...
    protected function setupSortOrder()
    {
        ...
        if ($this->Command != "json") {
            $defaultSort = $this->aisle->Expression . " ASC"; // Set up default sort
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->active_bin); // active_bin
            $this->updateSort($this->total_stock); // total_stock
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->aisle->setSort("");
                $this->active_bin->setSort("");
                $this->total_stock->setSort("");
            }
    ...
    public function renderListOptions()
    {
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"factive_stocklist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"factive_stocklist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->aisle->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("active_bin", $this->createColumnOption("active_bin"));
            $option->add("total_stock", $this->createColumnOption("total_stock"));
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"factive_stocksrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"factive_stocksrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="factive_stocklist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    protected function loadSearchValues()
    {
        ...
        // active_bin
        if ($this->active_bin->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->active_bin->AdvancedSearch->SearchValue != "" || $this->active_bin->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // total_stock
        if ($this->total_stock->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->total_stock->AdvancedSearch->SearchValue != "" || $this->total_stock->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        return $hasValue;
    ...
    public function loadRowValues($rs = null)
    {
        ...
        $this->active_bin->setDbValue($row['active_bin']);
    ...
    protected function newRow()
    {
        ...
        $row['active_bin'] = $this->active_bin->DefaultValue;
        $row['total_stock'] = $this->total_stock->DefaultValue;
        return $row;
    }
    ...
    public function renderRow()
    {
        ...
        // active_bin
        $this->active_bin->CellCssStyle = "white-space: nowrap;";

        // total_stock

        // Accumulate aggregate value
        if ($this->RowType != ROWTYPE_AGGREGATEINIT && $this->RowType != ROWTYPE_AGGREGATE && $this->RowType != ROWTYPE_PREVIEW_FIELD) {
            if (is_numeric($this->active_bin->CurrentValue)) {
                $this->active_bin->Total += $this->active_bin->CurrentValue; // Accumulate total
            }
            if (is_numeric($this->total_stock->CurrentValue)) {
                $this->total_stock->Total += $this->total_stock->CurrentValue; // Accumulate total
            }
        }
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // active_bin
            $this->active_bin->ViewValue = $this->active_bin->CurrentValue;
            $this->active_bin->ViewValue = FormatNumber($this->active_bin->ViewValue, $this->active_bin->formatPattern());
            $this->active_bin->ViewCustomAttributes = "";

            // total_stock
            $this->total_stock->ViewValue = $this->total_stock->CurrentValue;
            $this->total_stock->ViewValue = FormatNumber($this->total_stock->ViewValue, $this->total_stock->formatPattern());
            ...
            // active_bin
            $this->active_bin->LinkCustomAttributes = "";
            $this->active_bin->HrefValue = "";
            $this->active_bin->TooltipValue = "";
            ...
            // active_bin
            if ($this->active_bin->UseFilter && !EmptyValue($this->active_bin->AdvancedSearch->SearchValue)) {
                if (is_array($this->active_bin->AdvancedSearch->SearchValue)) {
                    $this->active_bin->AdvancedSearch->SearchValue = implode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->active_bin->AdvancedSearch->SearchValue);
                }
                $this->active_bin->EditValue = explode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->active_bin->AdvancedSearch->SearchValue);
            }

            // total_stock
            if ($this->total_stock->UseFilter && !EmptyValue($this->total_stock->AdvancedSearch->SearchValue)) {
                if (is_array($this->total_stock->AdvancedSearch->SearchValue)) {
                    $this->total_stock->AdvancedSearch->SearchValue = implode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->total_stock->AdvancedSearch->SearchValue);
                }
                $this->total_stock->EditValue = explode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->total_stock->AdvancedSearch->SearchValue);
            }
        } elseif ($this->RowType == ROWTYPE_AGGREGATEINIT) { // Initialize aggregate row
                    $this->active_bin->Total = 0; // Initialize total
                    $this->total_stock->Total = 0; // Initialize total
        } elseif ($this->RowType == ROWTYPE_AGGREGATE) { // Aggregate row
            $this->active_bin->CurrentValue = $this->active_bin->Total;
            $this->active_bin->ViewValue = $this->active_bin->CurrentValue;
            $this->active_bin->ViewValue = FormatNumber($this->active_bin->ViewValue, $this->active_bin->formatPattern());
            $this->active_bin->ViewCustomAttributes = "";
            $this->active_bin->HrefValue = ""; // Clear href value
            $this->total_stock->CurrentValue = $this->total_stock->Total;
            $this->total_stock->ViewValue = $this->total_stock->CurrentValue;
            $this->total_stock->ViewValue = FormatNumber($this->total_stock->ViewValue, $this->total_stock->formatPattern());
            $this->total_stock->ViewCustomAttributes = "";
            $this->total_stock->HrefValue = ""; // Clear href value
        }
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="factive_stocklist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"factive_stocksrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
