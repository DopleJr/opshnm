 ...
 * Page class
 */
class StockCountEdit extends StockCount
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "StockCountEdit";
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("stockcountlist"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "stockcountlist") {
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->article->IsDetailKey) {
            if (IsApi() && $val === null) {
                ...
                $this->article->setFormValue($val, true, $validate);
            }
        }
        ...
        if (!$this->time_created->IsDetailKey) {
            if (IsApi() && $val === null) {
                ...
                $this->time_created->setFormValue($val);
            }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->divisi->EditValue = HtmlEncode($this->divisi->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        if (!CheckInteger($this->article->FormValue)) {
            $this->article->addErrorMessage($this->article->getErrorMessage(false));
        }
        ...
        if ($this->divisi->Required) {
            if (!$this->divisi->IsDetailKey && EmptyValue($this->divisi->FormValue)) {
    ...
    protected function editRow()
    {
        ...
        // scan
        $this->scan->setDbValueDef($rsnew, $this->scan->CurrentValue, "", $this->scan->ReadOnly);
        ...
        // time_created
        $this->time_created->CurrentValue = CurrentTime();
        $this->time_created->setDbValueDef($rsnew, $this->time_created->CurrentValue, null);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("stockcountlist"), "", $this->TableVar, true);
        $pageId = "edit";
