...
class Mb51Edit extends Mb51
{
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("mb51list"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "mb51list") {
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'rcvsite' first before field var 'x_rcvsite'
        $val = $CurrentForm->hasValue("rcvsite") ? $CurrentForm->getValue("rcvsite") : $CurrentForm->getValue("x_rcvsite");
        if (!$this->rcvsite->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->rcvsite->Visible = false; // Disable update for API request
            } else {
                $this->rcvsite->setFormValue($val);
            }
        }

        // Check field name 'do_type' first before field var 'x_do_type'
        $val = $CurrentForm->hasValue("do_type") ? $CurrentForm->getValue("do_type") : $CurrentForm->getValue("x_do_type");
        if (!$this->do_type->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->do_type->Visible = false; // Disable update for API request
            } else {
                $this->do_type->setFormValue($val);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->rcvsite->CurrentValue = $this->rcvsite->FormValue;
        $this->do_type->CurrentValue = $this->do_type->FormValue;
    ...
    public function renderRow()
    {
        ...
        // rcvsite
        $this->rcvsite->RowCssClass = "row";

        // do_type
        $this->do_type->RowCssClass = "row";
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->rcvsite->EditValue = HtmlEncode($this->rcvsite->CurrentValue);
            ...
            $this->do_type->EditValue = HtmlEncode($this->do_type->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        if ($this->rcvsite->Required) {
            if (!$this->rcvsite->IsDetailKey && EmptyValue($this->rcvsite->FormValue)) {
                $this->rcvsite->addErrorMessage(str_replace("%s", $this->rcvsite->caption(), $this->rcvsite->RequiredErrorMessage));
            }
        }
        if ($this->do_type->Required) {
            if (!$this->do_type->IsDetailKey && EmptyValue($this->do_type->FormValue)) {
                $this->do_type->addErrorMessage(str_replace("%s", $this->do_type->caption(), $this->do_type->RequiredErrorMessage));
            }
        }
    ...
    protected function editRow()
    {
        ...
        // rcvsite
        $this->rcvsite->setDbValueDef($rsnew, $this->rcvsite->CurrentValue, null, $this->rcvsite->ReadOnly);

        // do_type
        $this->do_type->setDbValueDef($rsnew, $this->do_type->CurrentValue, null, $this->do_type->ReadOnly);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("mb51list"), "", $this->TableVar, true);
        $pageId = "edit";
