 ...
 * Page class
 */
class BoxPickingOnlineView extends BoxPickingOnline
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "BoxPickingOnlineView";
    ...
    public function run()
    {
        ...
        if ($this->isPageRequest()) { // Validate request
            if (($keyValue = Get("box_id") ?? Route("box_id")) !== null) {
                ...
                $this->RecKey["box_id"] = $this->box_id->QueryStringValue;
            } elseif (Post("box_id") !== null) {
                $this->box_id->setFormValue(Post("box_id"));
                $this->RecKey["box_id"] = $this->box_id->FormValue;
            } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
                $this->box_id->setQueryStringValue($keyValue);
                $this->RecKey["box_id"] = $this->box_id->QueryStringValue;
            } elseif (!$loadCurrentRecord) {
                $returnUrl = "boxpickingonlinelist"; // Return to list
            }
            ...
            switch ($this->CurrentAction) {
                case "show": // Get a record to display
                        ...
                        if (!$res) { // Load record based on key
                            ...
                            $returnUrl = "boxpickingonlinelist"; // No matching record, return to list
                        }
                    break;
            }
        } else {
            $returnUrl = "boxpickingonlinelist"; // Not page request, return to list
        }
        if ($returnUrl != "") {
