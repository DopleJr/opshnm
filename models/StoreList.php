...
class StoreList extends Store
{
    ...
    public function __construct()
    {
        ...
        // Table object (store)
        if (!isset($GLOBALS["store"]) || get_class($GLOBALS["store"]) == PROJECT_NAMESPACE . "store") {
            $GLOBALS["store"] = &$this;
        }
    ...
    public function run()
    {
        ...
        // Set up lookup cache
        $this->setupLookupOptions($this->totes);
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->totes->AdvancedSearch->toJson(), ","); // Field totes
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fstoresrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field store_code
        $this->store_code->AdvancedSearch->SearchValue = @$filter["x_store_code"];
        $this->store_code->AdvancedSearch->SearchOperator = @$filter["z_store_code"];
        $this->store_code->AdvancedSearch->SearchCondition = @$filter["v_store_code"];
        $this->store_code->AdvancedSearch->SearchValue2 = @$filter["y_store_code"];
        ...
        // Field totes
        $this->totes->AdvancedSearch->SearchValue = @$filter["x_totes"];
        $this->totes->AdvancedSearch->SearchOperator = @$filter["z_totes"];
        $this->totes->AdvancedSearch->SearchCondition = @$filter["v_totes"];
        $this->totes->AdvancedSearch->SearchValue2 = @$filter["y_totes"];
        $this->totes->AdvancedSearch->SearchOperator2 = @$filter["w_totes"];
        $this->totes->AdvancedSearch->save();
    ...
    protected function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->totes, $default, true); // totes
        ...
        if (!$default && $this->Command == "search") {
            ...
            $this->totes->AdvancedSearch->save(); // totes
        }
        return $where;
    }
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->totes->AdvancedSearch->issetSession()) {
            return true;
        }
        return false;
    }
    ...
    protected function resetAdvancedSearchParms()
    {
        ...
        $this->totes->AdvancedSearch->unsetSession();
    }
    ...
    protected function restoreSearchParms()
    {
        ...
        $this->totes->AdvancedSearch->load();
    }
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->totes); // totes
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fstorelist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fstorelist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<button type=\"button\" class=\"ew-action ew-multi-delete\" title=\"" . HtmlTitle($Language->phrase("DeleteSelectedLink")) . "\" data-caption=\"" . HtmlTitle($Language->phrase("DeleteSelectedLink")) . "\" form=\"fstorelist\" data-ew-action=\"submit\" data-url=\"" . GetUrl($this->MultiDeleteUrl) . "\"data-data='{\"action\":\"show\"}'>" . $Language->phrase("DeleteSelectedLink") . "</button>";
        $item->Visible = $Security->canDelete();
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("totes", $this->createColumnOption("totes"));
        }
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fstorelist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    protected function loadSearchValues()
    {
        ...
        // totes
        if ($this->totes->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->totes->AdvancedSearch->SearchValue != "" || $this->totes->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        return $hasValue;
    ...
    public function loadRowValues($rs = null)
    {
        ...
        $this->totes->setDbValue($row['totes']);
    }
    ...
    protected function newRow()
    {
        ...
        $row['totes'] = $this->totes->DefaultValue;
        return $row;
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // totes
            if (strval($this->totes->CurrentValue) != "") {
                $this->totes->ViewValue = $this->totes->optionCaption($this->totes->CurrentValue);
            } else {
                $this->totes->ViewValue = null;
            }
            $this->totes->ViewCustomAttributes = "";
            ...
            // totes
            $this->totes->LinkCustomAttributes = "";
            $this->totes->HrefValue = "";
            $this->totes->TooltipValue = "";
            ...
            // totes
            if ($this->totes->UseFilter && !EmptyValue($this->totes->AdvancedSearch->SearchValue)) {
                if (is_array($this->totes->AdvancedSearch->SearchValue)) {
                    $this->totes->AdvancedSearch->SearchValue = implode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->totes->AdvancedSearch->SearchValue);
                }
                $this->totes->EditValue = explode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->totes->AdvancedSearch->SearchValue);
            }
        }
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="fstorelist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fstoresrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
