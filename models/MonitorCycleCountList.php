 ...
 * Page class
 */
class MonitorCycleCountList extends MonitorCycleCount
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'monitor_cycle_count';

    // Page object name
    public $PageObjName = "MonitorCycleCountList";
    ...
    public $FormName = "fmonitor_cycle_countlist";
    ...
    public function __construct()
    {
        ...
        // Table object (monitor_cycle_count)
        if (!isset($GLOBALS["monitor_cycle_count"]) || get_class($GLOBALS["monitor_cycle_count"]) == PROJECT_NAMESPACE . "monitor_cycle_count") {
            $GLOBALS["monitor_cycle_count"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "monitorcyclecountadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "monitorcyclecountdelete";
        $this->MultiUpdateUrl = "monitorcyclecountupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'monitor_cycle_count');
        }
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $tbl = Container("monitor_cycle_count");
                $doc = new $class($tbl);
                $doc->Text = @$content;
    ...
    public function run()
    {
        ...
        $this->ShortArticle->setVisibility();
    ...
    public function getFilterList()
    {
        ...
        $filterList = "";
        $savedFilterList = "";
        $filterList = Concat($filterList, $this->DateCreated->AdvancedSearch->toJson(), ","); // Field Date Created
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fmonitor_cycle_countsrch", $filters);
    ...
    protected function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->ShortArticle, $default, true); // Short Article
        $this->buildSearchSql($where, $this->Total, $default, true); // Total
        $this->buildSearchSql($where, $this->User, $default, true); // User
        $this->buildSearchSql($where, $this->DateCreated, $default, true); // Date Created
        ...
        if (!$default && $this->Command == "search") {
            $this->DateCreated->AdvancedSearch->save(); // Date Created
        }
        return $where;
    }
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->ShortArticle->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    {
                $this->DateCreated->AdvancedSearch->loadDefault();
        return true;
    }
    ...
    protected function resetAdvancedSearchParms()
    {
        ...
        $this->ShortArticle->AdvancedSearch->unsetSession();
    ...
    protected function restoreSearchParms()
    {
        ...
        $this->ShortArticle->AdvancedSearch->load();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->ShortArticle); // Short Article
            $this->updateSort($this->Total); // Total
            $this->updateSort($this->User); // User
            $this->updateSort($this->DateCreated); // Date Created
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fmonitor_cycle_countlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fmonitor_cycle_countlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("Short Article", $this->createColumnOption("Short Article"));
            $option->add("Total", $this->createColumnOption("Total"));
            $option->add("User", $this->createColumnOption("User"));
            $option->add("Date Created", $this->createColumnOption("Date Created"));
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fmonitor_cycle_countsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fmonitor_cycle_countsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fmonitor_cycle_countlist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    protected function loadSearchValues()
    {
        ...
        // Short Article
        if ($this->ShortArticle->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->ShortArticle->AdvancedSearch->SearchValue != "" || $this->ShortArticle->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        // Date Created
        if ($this->DateCreated->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->DateCreated->AdvancedSearch->SearchValue != "" || $this->DateCreated->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        return $hasValue;
    ...
    public function loadRowValues($rs = null)
    {
        ...
        $this->ShortArticle->setDbValue($row['Short Article']);
        $this->Total->setDbValue($row['Total']);
        $this->User->setDbValue($row['User']);
        $this->DateCreated->setDbValue($row['Date Created']);
    }
    ...
    protected function newRow()
    {
        ...
        $row['Short Article'] = $this->ShortArticle->DefaultValue;
        $row['Total'] = $this->Total->DefaultValue;
        $row['User'] = $this->User->DefaultValue;
        $row['Date Created'] = $this->DateCreated->DefaultValue;
        return $row;
    }
    ...
    public function renderRow()
    {
        ...
        // Short Article
        $this->ShortArticle->CellCssStyle = "white-space: nowrap;";
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // Short Article
            $this->ShortArticle->ViewValue = $this->ShortArticle->CurrentValue;
            $this->ShortArticle->ViewCustomAttributes = "";
            ...
            $this->DateCreated->ViewValue = FormatDateTime($this->DateCreated->ViewValue, $this->DateCreated->formatPattern());
            ...
            // Short Article
            $this->ShortArticle->LinkCustomAttributes = "";
            $this->ShortArticle->HrefValue = "";
            $this->ShortArticle->TooltipValue = "";
            ...
            // Short Article
            if ($this->ShortArticle->UseFilter && !EmptyValue($this->ShortArticle->AdvancedSearch->SearchValue)) {
                if (is_array($this->ShortArticle->AdvancedSearch->SearchValue)) {
                    $this->ShortArticle->AdvancedSearch->SearchValue = implode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->ShortArticle->AdvancedSearch->SearchValue);
                }
                $this->ShortArticle->EditValue = explode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->ShortArticle->AdvancedSearch->SearchValue);
            }
            ...
            if ($this->Total->UseFilter && !EmptyValue($this->Total->AdvancedSearch->SearchValue)) {
                if (is_array($this->Total->AdvancedSearch->SearchValue)) {
                    $this->Total->AdvancedSearch->SearchValue = implode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->Total->AdvancedSearch->SearchValue);
                }
                $this->Total->EditValue = explode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->Total->AdvancedSearch->SearchValue);
            }
            ...
            // Date Created
            if ($this->DateCreated->UseFilter && !EmptyValue($this->DateCreated->AdvancedSearch->SearchValue)) {
                if (is_array($this->DateCreated->AdvancedSearch->SearchValue)) {
                    $this->DateCreated->AdvancedSearch->SearchValue = implode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->DateCreated->AdvancedSearch->SearchValue);
                }
                $this->DateCreated->EditValue = explode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->DateCreated->AdvancedSearch->SearchValue);
            }
            $this->DateCreated->setupEditAttributes();
            $this->DateCreated->EditCustomAttributes = "";
            $this->DateCreated->EditValue2 = HtmlEncode(FormatDateTime(UnFormatDateTime($this->DateCreated->AdvancedSearch->SearchValue2, $this->DateCreated->formatPattern()), $this->DateCreated->formatPattern()));
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="fmonitor_cycle_countlist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fmonitor_cycle_countsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
        ...
        if (IsMobile()) {
            $item->Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" . $Language->phrase("AdvancedSearch") . "\" data-caption=\"" . $Language->phrase("AdvancedSearch") . "\" href=\"monitorcyclecountsearch\">" . $Language->phrase("AdvancedSearchBtn") . "</a>";
        } else {
            $item->Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" . $Language->phrase("AdvancedSearch") . "\" data-table=\"monitor_cycle_count\" data-caption=\"" . $Language->phrase("AdvancedSearch") . "\" data-ew-action=\"modal\" data-url=\"monitorcyclecountsearch\" data-btn=\"SearchBtn\">" . $Language->phrase("AdvancedSearchBtn") . "</a>";
        }
        $item->Visible = true;
