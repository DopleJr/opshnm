 ...
 * Page class
 */
class VasValidationList extends VasValidation
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "VasValidationList";
    ...
    public $FormName = "fvas_validationlist";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "vasvalidationadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "vasvalidationdelete";
        $this->MultiUpdateUrl = "vasvalidationupdate";
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fvas_validationsrch", $filters);
    ...
    protected function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->po, $default, true); // po
        ...
        $this->buildSearchSql($where, $this->qty_oss, $default, true); // qty_oss
        ...
        $this->buildSearchSql($where, $this->price_foto, $default, true); // price_foto
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->qty_oss); // qty_oss
            $this->updateSort($this->shipment); // shipment
            $this->updateSort($this->aju); // aju
            $this->updateSort($this->actual_price); // actual_price
            $this->updateSort($this->price_foto); // price_foto
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fvas_validationlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fvas_validationlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<button type=\"button\" class=\"ew-action ew-multi-delete\" title=\"" . HtmlTitle($Language->phrase("DeleteSelectedLink")) . "\" data-caption=\"" . HtmlTitle($Language->phrase("DeleteSelectedLink")) . "\" form=\"fvas_validationlist\" data-ew-action=\"submit\" data-url=\"" . GetUrl($this->MultiDeleteUrl) . "\"data-data='{\"action\":\"show\"}'>" . $Language->phrase("DeleteSelectedLink") . "</button>";
        $item->Visible = $Security->canDelete();
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("po", $this->createColumnOption("po"));
            ...
            $option->add("qty_oss", $this->createColumnOption("qty_oss"));
            $option->add("shipment", $this->createColumnOption("shipment"));
            $option->add("aju", $this->createColumnOption("aju"));
            $option->add("actual_price", $this->createColumnOption("actual_price"));
            $option->add("price_foto", $this->createColumnOption("price_foto"));
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fvas_validationsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fvas_validationsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fvas_validationlist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($this->price_foto->UseColorbox) {
                ...
                $this->price_foto->LinkAttrs["data-rel"] = "vas_validation_x" . $this->RowCount . "_price_foto";
            ...
            // po
            if ($this->po->UseFilter && !EmptyValue($this->po->AdvancedSearch->SearchValue)) {
                if (is_array($this->po->AdvancedSearch->SearchValue)) {
                    $this->po->AdvancedSearch->SearchValue = implode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->po->AdvancedSearch->SearchValue);
                }
                $this->po->EditValue = explode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->po->AdvancedSearch->SearchValue);
            }
            ...
            // qty_oss
            if ($this->qty_oss->UseFilter && !EmptyValue($this->qty_oss->AdvancedSearch->SearchValue)) {
                if (is_array($this->qty_oss->AdvancedSearch->SearchValue)) {
                    $this->qty_oss->AdvancedSearch->SearchValue = implode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->qty_oss->AdvancedSearch->SearchValue);
                }
                $this->qty_oss->EditValue = explode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->qty_oss->AdvancedSearch->SearchValue);
            }
            ...
            // aju
            if ($this->aju->UseFilter && !EmptyValue($this->aju->AdvancedSearch->SearchValue)) {
                if (is_array($this->aju->AdvancedSearch->SearchValue)) {
                    $this->aju->AdvancedSearch->SearchValue = implode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->aju->AdvancedSearch->SearchValue);
                }
                $this->aju->EditValue = explode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->aju->AdvancedSearch->SearchValue);
            }
            ...
            // price_foto
            if ($this->price_foto->UseFilter && !EmptyValue($this->price_foto->AdvancedSearch->SearchValue)) {
                if (is_array($this->price_foto->AdvancedSearch->SearchValue)) {
                    $this->price_foto->AdvancedSearch->SearchValue = implode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->price_foto->AdvancedSearch->SearchValue);
                }
                $this->price_foto->EditValue = explode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->price_foto->AdvancedSearch->SearchValue);
            }
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="fvas_validationlist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fvas_validationsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
