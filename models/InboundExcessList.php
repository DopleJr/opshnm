 ...
 * Page class
 */
class InboundExcessList extends InboundExcess
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "InboundExcessList";
    ...
    public $FormName = "finbound_excesslist";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "inboundexcessadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "inboundexcessdelete";
        $this->MultiUpdateUrl = "inboundexcessupdate";
    ...
    public function run()
    {
        ...
        $this->total_scanned->Visible = false;
    ...
    public function getFilterList()
    {
        ...
        $filterList = "";
        $savedFilterList = "";
        $filterList = Concat($filterList, $this->total_scanned->AdvancedSearch->toJson(), ","); // Field total_scanned
        $filterList = Concat($filterList, $this->sscc->AdvancedSearch->toJson(), ","); // Field sscc
        $filterList = Concat($filterList, $this->pallet_id->AdvancedSearch->toJson(), ","); // Field pallet_id
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "finbound_excesssrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field total_scanned
        $this->total_scanned->AdvancedSearch->SearchValue = @$filter["x_total_scanned"];
        $this->total_scanned->AdvancedSearch->SearchOperator = @$filter["z_total_scanned"];
        $this->total_scanned->AdvancedSearch->SearchCondition = @$filter["v_total_scanned"];
        $this->total_scanned->AdvancedSearch->SearchValue2 = @$filter["y_total_scanned"];
        $this->total_scanned->AdvancedSearch->SearchOperator2 = @$filter["w_total_scanned"];
        $this->total_scanned->AdvancedSearch->save();
        ...
        // Field pallet_id
        $this->pallet_id->AdvancedSearch->SearchValue = @$filter["x_pallet_id"];
        $this->pallet_id->AdvancedSearch->SearchOperator = @$filter["z_pallet_id"];
        $this->pallet_id->AdvancedSearch->SearchCondition = @$filter["v_pallet_id"];
        $this->pallet_id->AdvancedSearch->SearchValue2 = @$filter["y_pallet_id"];
        $this->pallet_id->AdvancedSearch->SearchOperator2 = @$filter["w_pallet_id"];
        $this->pallet_id->AdvancedSearch->save();
    ...
    protected function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->total_scanned, $default, false); // total_scanned
        $this->buildSearchSql($where, $this->sscc, $default, true); // sscc
        $this->buildSearchSql($where, $this->pallet_id, $default, true); // pallet_id
        ...
        if (!$default && $this->Command == "search") {
            $this->total_scanned->AdvancedSearch->save(); // total_scanned
            $this->sscc->AdvancedSearch->save(); // sscc
            $this->pallet_id->AdvancedSearch->save(); // pallet_id
            $this->users->AdvancedSearch->save(); // users
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->total_scanned->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->sscc->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->pallet_id->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    {
        $this->total_scanned->AdvancedSearch->unsetSession();
        $this->sscc->AdvancedSearch->unsetSession();
        $this->pallet_id->AdvancedSearch->unsetSession();
    ...
    protected function restoreSearchParms()
    {
        ...
        $this->total_scanned->AdvancedSearch->load();
        $this->sscc->AdvancedSearch->load();
        $this->pallet_id->AdvancedSearch->load();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->pallet_id); // pallet_id
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->total_scanned->setSort("");
                $this->sscc->setSort("");
                $this->pallet_id->setSort("");
                $this->users->setSort("");
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"finbound_excesslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"finbound_excesslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<button type=\"button\" class=\"ew-action ew-multi-delete\" title=\"" . HtmlTitle($Language->phrase("DeleteSelectedLink")) . "\" data-caption=\"" . HtmlTitle($Language->phrase("DeleteSelectedLink")) . "\" form=\"finbound_excesslist\" data-ew-action=\"submit\" data-url=\"" . GetUrl($this->MultiDeleteUrl) . "\"data-data='{\"action\":\"show\"}'>" . $Language->phrase("DeleteSelectedLink") . "</button>";
        $item->Visible = $Security->canDelete();
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("pallet_id", $this->createColumnOption("pallet_id"));
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"finbound_excesssrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"finbound_excesssrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="finbound_excesslist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    protected function loadSearchValues()
    {
        ...
        // total_scanned
        if ($this->total_scanned->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->total_scanned->AdvancedSearch->SearchValue != "" || $this->total_scanned->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        // pallet_id
        if ($this->pallet_id->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->pallet_id->AdvancedSearch->SearchValue != "" || $this->pallet_id->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        if ($this->users->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->users->AdvancedSearch->SearchValue != "" || $this->users->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    public function renderRow()
    {
        ...
        // total_scanned
        $this->total_scanned->CellCssStyle = "white-space: nowrap;";

        // sscc
        $this->sscc->CellCssStyle = "white-space: nowrap;";

        // pallet_id
        $this->pallet_id->CellCssStyle = "white-space: nowrap;";
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // pallet_id
            if ($this->pallet_id->UseFilter && !EmptyValue($this->pallet_id->AdvancedSearch->SearchValue)) {
                if (is_array($this->pallet_id->AdvancedSearch->SearchValue)) {
                    $this->pallet_id->AdvancedSearch->SearchValue = implode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->pallet_id->AdvancedSearch->SearchValue);
                }
                $this->pallet_id->EditValue = explode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->pallet_id->AdvancedSearch->SearchValue);
            }
            ...
            if ($this->users->UseFilter && !EmptyValue($this->users->AdvancedSearch->SearchValue)) {
                if (is_array($this->users->AdvancedSearch->SearchValue)) {
                    $this->users->AdvancedSearch->SearchValue = implode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->users->AdvancedSearch->SearchValue);
                }
                $this->users->EditValue = explode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->users->AdvancedSearch->SearchValue);
            }
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="finbound_excesslist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"finbound_excesssrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
