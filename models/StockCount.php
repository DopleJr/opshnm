...
/**
 * Table class for stock_count
 */
class StockCount extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'stock_count';
        $this->TableName = 'stock_count';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "`stock_count`";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`stock_count`";
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("stockcountlist");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        } elseif ($pageName == "stockcountedit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "stockcountadd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            ...
            case Config("API_ADD_ACTION"):
                return "StockCountAdd";
            case Config("API_EDIT_ACTION"):
                return "StockCountEdit";
            case Config("API_DELETE_ACTION"):
                return "StockCountDelete";
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("stockcountview", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("stockcountview", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "stockcountadd?" . $this->getUrlParm($parm);
        } else {
            $url = "stockcountadd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        $url = $this->keyUrl("stockcountedit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("stockcountadd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("stockcountdelete", $this->getUrlParm());
    }
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->get_total);
                } else {
                    $doc->exportCaption($this->id);
                    $doc->exportCaption($this->location);
                    $doc->exportCaption($this->scan);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->get_total);
                    } else {
                        $doc->exportField($this->id);
                        $doc->exportField($this->location);
                        $doc->exportField($this->scan);
    ...
    public function rowInserting($rsold, &$rsnew)
    {
        ...
        // To cancel, set return value to false
            ...
            if($_location == "" || $_article == "" ){
            	$this->setWarningMessage("Tolong isi Lokasi & Article");
            	return false;	
            }else 
            {
            	return true;
    ...
    public function rowInserted($rsold, &$rsnew)
    {
        ...
        $sql4 = "UPDATE stock_count SET `date_created` = '$currentDate', `divisi` = '$_divisi' , `user` = '$currentUserr' WHERE `id` = '".$rsnew["id"]."' ";
