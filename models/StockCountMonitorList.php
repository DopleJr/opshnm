 ...
 * Page class
 */
class StockCountMonitorList extends StockCountMonitor
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'stock_count_monitor';

    // Page object name
    public $PageObjName = "StockCountMonitorList";
    ...
    public $FormName = "fstock_count_monitorlist";
    ...
    public function __construct()
    {
        ...
        // Table object (stock_count_monitor)
        if (!isset($GLOBALS["stock_count_monitor"]) || get_class($GLOBALS["stock_count_monitor"]) == PROJECT_NAMESPACE . "stock_count_monitor") {
            $GLOBALS["stock_count_monitor"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "stockcountmonitoradd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "stockcountmonitordelete";
        $this->MultiUpdateUrl = "stockcountmonitorupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'stock_count_monitor');
        }
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $tbl = Container("stock_count_monitor");
                $doc = new $class($tbl);
                $doc->Text = @$content;
    ...
    public $PageSizes = "10,20,50,100,200"; // Page sizes (comma separated)
    ...
    public function run()
    {
        ...
        $this->counted_location->setVisibility();
        $this->counted_pcs->setVisibility();
        $this->progress->setVisibility();
        $this->total_stock->Visible = false;
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->counted_location->AdvancedSearch->toJson(), ","); // Field counted_location
        $filterList = Concat($filterList, $this->counted_pcs->AdvancedSearch->toJson(), ","); // Field counted_pcs
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fstock_count_monitorsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field counted_location
        $this->counted_location->AdvancedSearch->SearchValue = @$filter["x_counted_location"];
        $this->counted_location->AdvancedSearch->SearchOperator = @$filter["z_counted_location"];
        $this->counted_location->AdvancedSearch->SearchCondition = @$filter["v_counted_location"];
        $this->counted_location->AdvancedSearch->SearchValue2 = @$filter["y_counted_location"];
        $this->counted_location->AdvancedSearch->SearchOperator2 = @$filter["w_counted_location"];
        $this->counted_location->AdvancedSearch->save();

        // Field counted_pcs
        $this->counted_pcs->AdvancedSearch->SearchValue = @$filter["x_counted_pcs"];
        $this->counted_pcs->AdvancedSearch->SearchOperator = @$filter["z_counted_pcs"];
        $this->counted_pcs->AdvancedSearch->SearchCondition = @$filter["v_counted_pcs"];
        $this->counted_pcs->AdvancedSearch->SearchValue2 = @$filter["y_counted_pcs"];
        $this->counted_pcs->AdvancedSearch->SearchOperator2 = @$filter["w_counted_pcs"];
        $this->counted_pcs->AdvancedSearch->save();
    ...
    protected function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->counted_location, $default, true); // counted_location
        $this->buildSearchSql($where, $this->counted_pcs, $default, true); // counted_pcs
        $this->buildSearchSql($where, $this->progress, $default, false); // progress
        ...
        if (!$default && $this->Command == "search") {
            ...
            $this->counted_location->AdvancedSearch->save(); // counted_location
            $this->counted_pcs->AdvancedSearch->save(); // counted_pcs
    ...
    protected function basicSearchWhere($default = false)
    {
        ...
        $searchFlds[] = &$this->counted_location;
        $searchFlds[] = &$this->counted_pcs;
        $searchFlds[] = &$this->progress;
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->counted_location->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->counted_pcs->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    protected function resetAdvancedSearchParms()
    {
        ...
        $this->counted_location->AdvancedSearch->unsetSession();
        $this->counted_pcs->AdvancedSearch->unsetSession();
    ...
    protected function restoreSearchParms()
    {
        ...
        $this->counted_location->AdvancedSearch->load();
        $this->counted_pcs->AdvancedSearch->load();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->counted_location); // counted_location
            $this->updateSort($this->counted_pcs); // counted_pcs
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->aisle->setSort("");
                $this->counted_location->setSort("");
                $this->counted_pcs->setSort("");
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fstock_count_monitorlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fstock_count_monitorlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("counted_location", $this->createColumnOption("counted_location"));
            $option->add("counted_pcs", $this->createColumnOption("counted_pcs"));
            $option->add("progress", $this->createColumnOption("progress"));
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fstock_count_monitorsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fstock_count_monitorsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fstock_count_monitorlist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    protected function loadSearchValues()
    {
        ...
        // counted_location
        if ($this->counted_location->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->counted_location->AdvancedSearch->SearchValue != "" || $this->counted_location->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // counted_pcs
        if ($this->counted_pcs->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->counted_pcs->AdvancedSearch->SearchValue != "" || $this->counted_pcs->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    protected function newRow()
    {
        ...
        $row['counted_location'] = $this->counted_location->DefaultValue;
        $row['counted_pcs'] = $this->counted_pcs->DefaultValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // counted_location
            if ($this->counted_location->UseFilter && !EmptyValue($this->counted_location->AdvancedSearch->SearchValue)) {
                if (is_array($this->counted_location->AdvancedSearch->SearchValue)) {
                    $this->counted_location->AdvancedSearch->SearchValue = implode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->counted_location->AdvancedSearch->SearchValue);
                }
                $this->counted_location->EditValue = explode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->counted_location->AdvancedSearch->SearchValue);
            }

            // counted_pcs
            if ($this->counted_pcs->UseFilter && !EmptyValue($this->counted_pcs->AdvancedSearch->SearchValue)) {
                if (is_array($this->counted_pcs->AdvancedSearch->SearchValue)) {
                    $this->counted_pcs->AdvancedSearch->SearchValue = implode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->counted_pcs->AdvancedSearch->SearchValue);
                }
                $this->counted_pcs->EditValue = explode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->counted_pcs->AdvancedSearch->SearchValue);
            }

            // progress
            $this->progress->setupEditAttributes();
            $this->progress->EditCustomAttributes = "";
            $this->progress->EditValue = HtmlEncode($this->progress->AdvancedSearch->SearchValue);
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="fstock_count_monitorlist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fstock_count_monitorsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
