...
class StoreEdit extends Store
{
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'totes' first before field var 'x_totes'
        $val = $CurrentForm->hasValue("totes") ? $CurrentForm->getValue("totes") : $CurrentForm->getValue("x_totes");
        if (!$this->totes->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->totes->Visible = false; // Disable update for API request
            } else {
                $this->totes->setFormValue($val);
            }
        }
    }
    ...
    public function restoreFormValues()
    {
        ...
        $this->totes->CurrentValue = $this->totes->FormValue;
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // totes
            $this->totes->setupEditAttributes();
            $this->totes->EditCustomAttributes = "";
            $this->totes->EditValue = $this->totes->options(true);
            $this->totes->PlaceHolder = RemoveHtml($this->totes->caption());
    ...
    protected function validateForm()
    {
        ...
        if ($this->totes->Required) {
            if (!$this->totes->IsDetailKey && EmptyValue($this->totes->FormValue)) {
                $this->totes->addErrorMessage(str_replace("%s", $this->totes->caption(), $this->totes->RequiredErrorMessage));
            }
        }
    ...
    protected function editRow()
    {
        ...
        // totes
        $this->totes->setDbValueDef($rsnew, $this->totes->CurrentValue, null, $this->totes->ReadOnly);
