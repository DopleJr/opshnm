 ...
 * Page class
 */
class FindingshortpickList extends Findingshortpick
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "FindingshortpickList";
    ...
    public $FormName = "ffindingshortpicklist";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "findingshortpickadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "findingshortpickdelete";
        $this->MultiUpdateUrl = "findingshortpickupdate";
    ...
    protected function hideFieldsForAddEdit()
    {
        ...
        if ($this->isAddOrEdit()) {
            $this->date_picking->Visible = false;
        }
    }
    ...
    public $PageSizes = "10,20,50,100,500"; // Page sizes (comma separated)
    ...
    public function run()
    {
        ...
        $this->total_shortpick->Visible = false;
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->target_quantity->AdvancedSearch->toJson(), ","); // Field target_quantity
        $filterList = Concat($filterList, $this->pick_quantity->AdvancedSearch->toJson(), ","); // Field pick_quantity
        $filterList = Concat($filterList, $this->shortpick->AdvancedSearch->toJson(), ","); // Field shortpick
        ...
        $filterList = Concat($filterList, $this->short_article->AdvancedSearch->toJson(), ","); // Field short_article
        $filterList = Concat($filterList, $this->total->AdvancedSearch->toJson(), ","); // Field total
        $filterList = Concat($filterList, $this->excess->AdvancedSearch->toJson(), ","); // Field excess
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "ffindingshortpicksrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        $this->target_quantity->AdvancedSearch->SearchValue = @$filter["x_target_quantity"];
        $this->target_quantity->AdvancedSearch->SearchOperator = @$filter["z_target_quantity"];
        $this->target_quantity->AdvancedSearch->SearchCondition = @$filter["v_target_quantity"];
        $this->target_quantity->AdvancedSearch->SearchValue2 = @$filter["y_target_quantity"];
        $this->target_quantity->AdvancedSearch->SearchOperator2 = @$filter["w_target_quantity"];
        $this->target_quantity->AdvancedSearch->save();

        // Field pick_quantity
        $this->pick_quantity->AdvancedSearch->SearchValue = @$filter["x_pick_quantity"];
        $this->pick_quantity->AdvancedSearch->SearchOperator = @$filter["z_pick_quantity"];
        $this->pick_quantity->AdvancedSearch->SearchCondition = @$filter["v_pick_quantity"];
        $this->pick_quantity->AdvancedSearch->SearchValue2 = @$filter["y_pick_quantity"];
        $this->pick_quantity->AdvancedSearch->SearchOperator2 = @$filter["w_pick_quantity"];
        $this->pick_quantity->AdvancedSearch->save();

        // Field shortpick
        $this->shortpick->AdvancedSearch->SearchValue = @$filter["x_shortpick"];
        $this->shortpick->AdvancedSearch->SearchOperator = @$filter["z_shortpick"];
        $this->shortpick->AdvancedSearch->SearchCondition = @$filter["v_shortpick"];
        $this->shortpick->AdvancedSearch->SearchValue2 = @$filter["y_shortpick"];
        $this->shortpick->AdvancedSearch->SearchOperator2 = @$filter["w_shortpick"];
        $this->shortpick->AdvancedSearch->save();
        ...
        // Field short_article
        $this->short_article->AdvancedSearch->SearchValue = @$filter["x_short_article"];
        $this->short_article->AdvancedSearch->SearchOperator = @$filter["z_short_article"];
        $this->short_article->AdvancedSearch->SearchCondition = @$filter["v_short_article"];
        $this->short_article->AdvancedSearch->SearchValue2 = @$filter["y_short_article"];
        $this->short_article->AdvancedSearch->SearchOperator2 = @$filter["w_short_article"];
        $this->short_article->AdvancedSearch->save();
        ...
        // Field excess
        $this->excess->AdvancedSearch->SearchValue = @$filter["x_excess"];
        $this->excess->AdvancedSearch->SearchOperator = @$filter["z_excess"];
        $this->excess->AdvancedSearch->SearchCondition = @$filter["v_excess"];
        $this->excess->AdvancedSearch->SearchValue2 = @$filter["y_excess"];
        $this->excess->AdvancedSearch->SearchOperator2 = @$filter["w_excess"];
        $this->excess->AdvancedSearch->save();
    ...
    protected function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->total_shortpick, $default, false); // total_shortpick
        $this->buildSearchSql($where, $this->short_article, $default, false); // short_article
        $this->buildSearchSql($where, $this->total, $default, false); // total
        $this->buildSearchSql($where, $this->excess, $default, false); // excess
        ...
        if (!$default && $this->Command == "search") {
            ...
            $this->target_quantity->AdvancedSearch->save(); // target_quantity
            $this->pick_quantity->AdvancedSearch->save(); // pick_quantity
            $this->shortpick->AdvancedSearch->save(); // shortpick
            ...
            $this->short_article->AdvancedSearch->save(); // short_article
            $this->total->AdvancedSearch->save(); // total
            $this->excess->AdvancedSearch->save(); // excess
        }
        return $where;
    }
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->short_article->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    protected function resetAdvancedSearchParms()
    {
        ...
        $this->short_article->AdvancedSearch->unsetSession();
    ...
    protected function restoreSearchParms()
    {
        ...
        $this->short_article->AdvancedSearch->load();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->target_quantity); // target_quantity
            $this->updateSort($this->pick_quantity); // pick_quantity
            $this->updateSort($this->shortpick); // shortpick
            $this->updateSort($this->user); // user
            $this->updateSort($this->area); // area
            $this->updateSort($this->status); // status
            $this->updateSort($this->date_upload); // date_upload
            $this->updateSort($this->date_picking); // date_picking
            $this->updateSort($this->short_article); // short_article
            $this->updateSort($this->total); // total
            $this->updateSort($this->excess); // excess
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"ffindingshortpicklist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"ffindingshortpicklist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("short_article", $this->createColumnOption("short_article"));
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"ffindingshortpicksrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"ffindingshortpicksrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="ffindingshortpicklist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    protected function loadSearchValues()
    {
        ...
        // short_article
        if ($this->short_article->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->short_article->AdvancedSearch->SearchValue != "" || $this->short_article->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->short_article->TooltipValue = "";
            ...
            if (!$this->short_article->Raw) {
                $this->short_article->AdvancedSearch->SearchValue = HtmlDecode($this->short_article->AdvancedSearch->SearchValue);
            }
            $this->short_article->EditValue = HtmlEncode($this->short_article->AdvancedSearch->SearchValue);
            ...
            $this->excess->EditValue = HtmlEncode($this->excess->AdvancedSearch->SearchValue);
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"ffindingshortpicksrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    */
    ...
    public function pageRedirecting(&$url)
    {
        // Example:
        //$url = "your URL";
        if (Page('findingshortpick')->id->CurrentValue == null) {// List page with null record
            	 // assume your ID field is number so no need to URL-encode
            	echo "<script>alert('Job has complete!!')</script>";
                $url = "shortpickbyarealist";    
            	}
        	else{
            $url = "findingshortpickedit?start=1";// List page has record
            }
    }
