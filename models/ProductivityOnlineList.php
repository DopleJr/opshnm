 ...
 * Page class
 */
class ProductivityOnlineList extends ProductivityOnline
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'productivity_online';

    // Page object name
    public $PageObjName = "ProductivityOnlineList";
    ...
    public $FormName = "fproductivity_onlinelist";
    ...
    public function __construct()
    {
        ...
        // Table object (productivity_online)
        if (!isset($GLOBALS["productivity_online"]) || get_class($GLOBALS["productivity_online"]) == PROJECT_NAMESPACE . "productivity_online") {
            $GLOBALS["productivity_online"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "productivityonlineadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "productivityonlinedelete";
        $this->MultiUpdateUrl = "productivityonlineupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'productivity_online');
        }
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $tbl = Container("productivity_online");
                $doc = new $class($tbl);
                $doc->Text = @$content;
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fproductivity_onlinesrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        $this->picking_date->AdvancedSearch->SearchCondition = @$filter["v_picking_date"];
        ...
        // Field picked
        $this->picked->AdvancedSearch->SearchValue = @$filter["x_picked"];
        $this->picked->AdvancedSearch->SearchOperator = @$filter["z_picked"];
        $this->picked->AdvancedSearch->SearchCondition = @$filter["v_picked"];
        $this->picked->AdvancedSearch->SearchValue2 = @$filter["y_picked"];
        $this->picked->AdvancedSearch->SearchOperator2 = @$filter["w_picked"];
        $this->picked->AdvancedSearch->save();
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->picked->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    protected function setupSortOrder()
    {
        ...
        if ($this->Command != "json") {
            $defaultSort = $this->picking_date->Expression . " DESC" . ", " . $this->total->Expression . " DESC"; // Set up default sort
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fproductivity_onlinelist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fproductivity_onlinelist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fproductivity_onlinesrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fproductivity_onlinesrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fproductivity_onlinelist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    protected function loadSearchValues()
    {
        ...
        // picking_date
        if ($this->picking_date->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->picking_date->AdvancedSearch->SearchValue != "" || $this->picking_date->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        // picked
        if ($this->picked->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->picked->AdvancedSearch->SearchValue != "" || $this->picked->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // variance
        if ($this->variance->AdvancedSearch->get()) {
            $hasValue = true;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // picked
            if ($this->picked->UseFilter && !EmptyValue($this->picked->AdvancedSearch->SearchValue)) {
                if (is_array($this->picked->AdvancedSearch->SearchValue)) {
                    $this->picked->AdvancedSearch->SearchValue = implode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->picked->AdvancedSearch->SearchValue);
                }
                $this->picked->EditValue = explode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->picked->AdvancedSearch->SearchValue);
            }

            // variance
            if ($this->variance->UseFilter && !EmptyValue($this->variance->AdvancedSearch->SearchValue)) {
                if (is_array($this->variance->AdvancedSearch->SearchValue)) {
                    $this->variance->AdvancedSearch->SearchValue = implode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->variance->AdvancedSearch->SearchValue);
                }
                $this->variance->EditValue = explode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->variance->AdvancedSearch->SearchValue);
            }
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="fproductivity_onlinelist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fproductivity_onlinesrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
