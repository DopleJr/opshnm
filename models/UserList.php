...
class UserList extends User
{
    ...
    protected function restoreFilterList()
    {
        ...
        // Field ip_loggedin
        $this->ip_loggedin->AdvancedSearch->SearchValue = @$filter["x_ip_loggedin"];
        $this->ip_loggedin->AdvancedSearch->SearchOperator = @$filter["z_ip_loggedin"];
        $this->ip_loggedin->AdvancedSearch->SearchCondition = @$filter["v_ip_loggedin"];
        $this->ip_loggedin->AdvancedSearch->SearchValue2 = @$filter["y_ip_loggedin"];
        $this->ip_loggedin->AdvancedSearch->SearchOperator2 = @$filter["w_ip_loggedin"];
        $this->ip_loggedin->AdvancedSearch->save();
    ...
    protected function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->_username, $default, true); // username
        $this->buildSearchSql($where, $this->_password, $default, true); // password
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->ip_loggedin->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<button type=\"button\" class=\"ew-action ew-multi-delete\" title=\"" . HtmlTitle($Language->phrase("DeleteSelectedLink")) . "\" data-caption=\"" . HtmlTitle($Language->phrase("DeleteSelectedLink")) . "\" form=\"fuserlist\" data-ew-action=\"submit\" data-url=\"" . GetUrl($this->MultiDeleteUrl) . "\"data-data='{\"action\":\"show\"}'>" . $Language->phrase("DeleteSelectedLink") . "</button>";
        $item->Visible = $Security->canDelete();
    ...
    protected function loadSearchValues()
    {
        ...
        // ip_loggedin
        if ($this->ip_loggedin->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->ip_loggedin->AdvancedSearch->SearchValue != "" || $this->ip_loggedin->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // role
        if ($this->role->AdvancedSearch->get()) {
            $hasValue = true;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // username
            if ($this->_username->UseFilter && !EmptyValue($this->_username->AdvancedSearch->SearchValue)) {
                if (is_array($this->_username->AdvancedSearch->SearchValue)) {
                    $this->_username->AdvancedSearch->SearchValue = implode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->_username->AdvancedSearch->SearchValue);
                }
            ...
            if ($this->ip_loggedin->UseFilter && !EmptyValue($this->ip_loggedin->AdvancedSearch->SearchValue)) {
                if (is_array($this->ip_loggedin->AdvancedSearch->SearchValue)) {
                    $this->ip_loggedin->AdvancedSearch->SearchValue = implode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->ip_loggedin->AdvancedSearch->SearchValue);
                }
                $this->ip_loggedin->EditValue = explode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->ip_loggedin->AdvancedSearch->SearchValue);
            }

            // role
            if ($this->role->UseFilter && !EmptyValue($this->role->AdvancedSearch->SearchValue)) {
                if (is_array($this->role->AdvancedSearch->SearchValue)) {
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="fuserlist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
