...
loadjs.ready(["wrapper", "head"], function () {
    ...
    fpickingadd.lists.status = <?= $Page->status->toClientList($Page) ?>;
...
<span id="el_picking_status">
    <select
        ...
        data-select2-id="fpickingadd_x_status"
        data-table="picking"
        data-field="x_status"
...
<script>
loadjs.ready("fpickingadd", function() {
    var options = { name: "x_status", selectId: "fpickingadd_x_status" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fpickingadd.lists.status.lookupOptions.length) {
        options.data = { id: "x_status", form: "fpickingadd" };
    } else {
        options.ajax = { id: "x_status", form: "fpickingadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
