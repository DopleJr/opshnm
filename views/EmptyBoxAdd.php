...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fempty_boxadd = new ew.Form("fempty_boxadd", "add");
    currentPageID = ew.PAGE_ID = "add";
    currentForm = fempty_boxadd;

    // Add fields
    var fields = currentTable.fields;
    fempty_boxadd.addFields([
    ...
    // Form_CustomValidate
    fempty_boxadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fempty_boxadd.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fempty_boxadd.lists.divisi = <?= $Page->divisi->toClientList($Page) ?>;
    loadjs.done("fempty_boxadd");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fempty_boxadd" id="fempty_boxadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
