...
loadjs.ready(["wrapper", "head"], function () {
    ...
    fuserlevelslist.lists.userlevelid = <?= $Page->userlevelid->toClientList($Page) ?>;
    fuserlevelslist.lists.userlevelname = <?= $Page->userlevelname->toClientList($Page) ?>;
...
loadjs.ready(["wrapper", "head"], function () {
    ...
    // Form_CustomValidate
    fuserlevelssrch.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fuserlevelssrch.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fuserlevelssrch.lists.userlevelid = <?= $Page->userlevelid->toClientList($Page) ?>;
    fuserlevelssrch.lists.userlevelname = <?= $Page->userlevelname->toClientList($Page) ?>;
...
?>
    <div id="xs_userlevelid" class="col-sm-auto d-sm-flex mb-3 px-0 pe-sm-2<?= $Page->userlevelid->UseFilter ? " ew-filter-field" : "" ?>">
        <select
            ...
            data-select2-id="fuserlevelssrch_x_userlevelid"
        ...
        loadjs.ready("fuserlevelssrch", function() {
            var options = {
                name: "x_userlevelid",
                selectId: "fuserlevelssrch_x_userlevelid",
                ajax: { id: "x_userlevelid", form: "fuserlevelssrch", limit: ew.FILTER_PAGE_SIZE, data: { ajax: "filter" } }
            };
            options = Object.assign({}, ew.filterOptions, options, ew.vars.tables.userlevels.fields.userlevelid.filterOptions);
...
<?php
if (!$Page->userlevelname->UseFilter) {
    $Page->SearchColumnCount++;
}
?>
    <div id="xs_userlevelname" class="col-sm-auto d-sm-flex mb-3 px-0 pe-sm-2<?= $Page->userlevelname->UseFilter ? " ew-filter-field" : "" ?>">
        <select
            id="x_userlevelname"
            name="x_userlevelname[]"
            class="form-control ew-select<?= $Page->userlevelname->isInvalidClass() ?>"
            data-select2-id="fuserlevelssrch_x_userlevelname"
            data-table="userlevels"
            data-field="x_userlevelname"
            data-caption="<?= HtmlEncode(RemoveHtml($Page->userlevelname->caption())) ?>"
            data-filter="true"
            multiple
            size="1"
            data-value-separator="<?= $Page->userlevelname->displayValueSeparatorAttribute() ?>"
            data-placeholder="<?= HtmlEncode($Page->userlevelname->getPlaceHolder()) ?>"
            <?= $Page->userlevelname->editAttributes() ?>>
            <?= $Page->userlevelname->selectOptionListHtml("x_userlevelname", true) ?>
        </select>
        ...
        loadjs.ready("fuserlevelssrch", function() {
            var options = {
                name: "x_userlevelname",
                selectId: "fuserlevelssrch_x_userlevelname",
                ajax: { id: "x_userlevelname", form: "fuserlevelssrch", limit: ew.FILTER_PAGE_SIZE, data: { ajax: "filter" } }
            };
            options = Object.assign({}, ew.filterOptions, options, ew.vars.tables.userlevels.fields.userlevelname.filterOptions);
            ew.createFilter(options);
        });
        </script>
    </div><!-- /.col-sm-auto -->
...
?>
<?php if ($Page->userlevelid->Visible) { // userlevelid ?>
        <th data-name="userlevelid" class="<?= $Page->userlevelid->headerCellClass() ?>" style="white-space: nowrap;"><div id="elh_userlevels_userlevelid" class="userlevels_userlevelid"><?= $Page->renderFieldHeader($Page->userlevelid) ?></div></th>
<?php } ?>
<?php if ($Page->userlevelname->Visible) { // userlevelname ?>
        <th data-name="userlevelname" class="<?= $Page->userlevelname->headerCellClass() ?>" style="white-space: nowrap;"><div id="elh_userlevels_userlevelname" class="userlevels_userlevelname"><?= $Page->renderFieldHeader($Page->userlevelname) ?></div></th>
<?php } ?>
<?php
