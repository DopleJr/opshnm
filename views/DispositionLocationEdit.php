...
// Page object
$DispositionLocationEdit = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { disposition_location: currentTable } });
var currentForm, currentPageID;
var fdisposition_locationedit;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fdisposition_locationedit = new ew.Form("fdisposition_locationedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fdisposition_locationedit;

    // Add fields
    var fields = currentTable.fields;
    fdisposition_locationedit.addFields([
    ...
    // Form_CustomValidate
    fdisposition_locationedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fdisposition_locationedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fdisposition_locationedit.lists.divisi = <?= $Page->divisi->toClientList($Page) ?>;
    loadjs.done("fdisposition_locationedit");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fdisposition_locationedit" id="fdisposition_locationedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id"<?= $Page->id->rowAttributes() ?>>
        <label id="elh_disposition_location_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->id->cellAttributes() ?>>
<span id="el_disposition_location_id">
<span<?= $Page->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id->getDisplayValue($Page->id->EditValue))) ?>"></span>
<input type="hidden" data-table="disposition_location" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<?php if ($Page->photo_after->Visible) { // photo_after ?>
    <div id="r_photo_after"<?= $Page->photo_after->rowAttributes() ?>>
        <label id="elh_disposition_location_photo_after" class="<?= $Page->LeftColumnClass ?>"><?= $Page->photo_after->caption() ?><?= $Page->photo_after->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
